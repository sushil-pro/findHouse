...
if (!class_exists('adg_custom_post_types')) {
    class adg_custom_post_types
    {
        ...
        public function update_theme_compatibility_template($themes)
        {
            ...
            // v 2.1.18
            $themes['pointfinder'] = array(
                'single_template' => get_template_directory() . '/single.php',
                'replace_type' => 'preg',
                'replace' => array('#\$post_type(.+)get_footer()#s'),
                'after' => "echo '</div></div></div></section>'; \n} \nget_footer",
                'before' => "echo '<section role=\"main\"><div class=\"pf-container\"><div class=\"pf-row\"><div class=\"col-lg-12\">';\n",
            );
        ...
        public function init_created_cpt()
        {
            ...
            foreach ($created_post_types as $post_type) {
                ...
                $supports = array('title', 'page-attributes', 'author', 'custom-fields', 'comments');
        ...
        public function init_created_tax()
        {
            ...
            foreach ($created_taxonomies as $taxonomy) {
                ...
                $args = array(
                    ...
                    'rewrite'           => $tax_settings['tax_public'] == 0 ? false : array('slug' => sanitize_title($taxonomy->post_title), 'with_front' => false, 'hierarchical' => true),
            ...
            $this->flush_rewrite_rules($created_taxonomies);
        }
        ...
        public function copy_theme_single_template()
        {
            ...
            if (!file_exists($current_teme_single_template) && (isset($_GET['action']) && 'copy_theme_template' == $_GET['action'])) {
                ...
                $add_before = isset($template_to_copy['before']) ? $template_to_copy['before'] : '';
                $add_after = isset($template_to_copy['after']) ? $template_to_copy['after'] : '';
                if ($template_to_copy['replace_type'] == 'string') {
                    $new_single_template = str_replace($template_to_copy['replace'], $add_before . "custom_post_types_get_custom_template()" . $add_after, $theme_single_template);
                    $copy_template = true;
                } else {
                    if (!is_array($template_to_copy['replace'])) {
                        if (preg_match($template_to_copy['replace'], $theme_single_template, $matches)) {
                            $new_single_template = preg_replace($template_to_copy['replace'], $add_before . "custom_post_types_get_custom_template();\n" . $add_after, $theme_single_template);
                            $copy_template = true;
                        }
                    } else {
                        foreach ($template_to_copy['replace'] as $replace_regex) {
                            if (!$copy_template && preg_match($replace_regex, $theme_single_template, $matches)) {
                                $new_single_template = preg_replace($replace_regex, $add_before . "custom_post_types_get_custom_template();\n" . $add_after, $theme_single_template);
                                $copy_template = true;
                            }
                        }
                    }
